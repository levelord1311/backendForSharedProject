...
var (
	ErrNotFound = NewAppError(nil, "not found", "", "REAS-003000")
)
...
func systemError(err error) *AppError {
	return NewAppError(err, "system error", err.Error(), "REAS-000001")
}

func BadRequestError(message, developerMessage string) *AppError {
	return NewAppError(fmt.Errorf(message), message, developerMessage, "REAS-000002")
}

func UnauthorizedError(message string) *AppError {
	return NewAppError(fmt.Errorf(message), message, "", "REAS-000003")
}
